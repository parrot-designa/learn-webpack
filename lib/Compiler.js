	...
	runAsChild(callback) {
		...
		const finalCallback = (err, entries, compilation) => {
			try {
				...
				err.details =  (runAsChildErr).stack;
...
${other}`);
						...
						const updateWithReplacementSource = size => {
							...
							if (
								 (similarEntry).waiting !== undefined
							) {
								for (const { file, cacheEntry } of  (
									similarEntry
								).waiting) {
						...
						const processExistingFile = stats => {
							...
							if (content.length === stats.size) {
								...
								return  (
									...
									if (
										err ||
										!content.equals( (existingContent))
									) {
						...
						if (targetFileGeneration !== undefined) {
							...
							if (writtenGeneration === targetFileGeneration) {
								...
								if (this._assetEmittingPreviousFiles.has(targetPath)) {
									const sizeOnlySource =  (
										 (cacheEntry).sizeOnlySource
									);
			...
			(this.inputFileSystem).readFile(
				...
				(err, content) => {
					...
					try {
						...
						return callback(
							new Error(
								`Cannot parse records: ${ (parseErr).message}`
							)
						);
					}
